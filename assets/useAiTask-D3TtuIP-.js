...
`;try{return console.log("📤 发送 API 请求，使用系统指令:",u.substring(0,100)+"..."),(await _(m,s,[{role:"system",content:u},{role:"user",content:d}],{temperature:.75,signal:p==null?void 0:p.signal,timeout:12e4,maxRetries:1})).text||"生成内容为空"}catch(r){throw console.error("Script Generation Error:",r),r}},J=async($,e,s,c,p="[次]gemini-3-flash-preview",m,u,o)=>{const a=E($,m,p),i=c.slice(-5).map(r=>`${r.role==="user"?"用户":"模型"}: ${r.text} ${r.quotedText?`(引用: "${r.quotedText.substring(0,30)}...")`:""}`).join(`
`),t=u?`
【用户特意选中的待修改片段】：
"${u}"
...
`;try{const h=(await _(a,p,[{role:"user",content:d}],{temperature:.7,timeout:12e4,maxRetries:1})).text||"",n=h.match(/<SCRIPT_START>([\s\S]*?)<SCRIPT_END>/);let S=null,y=h;return n&&(S=n[1].trim(),y=h.replace(/<SCRIPT_START>[\s\S]*?<SCRIPT_END>/,"").trim(),y||(y="已根据您的要求修改了剧本。")),{textResponse:y,newScript:S}}catch(r){throw console.error("Refine Script Error:",r),r}},Z=async($,e,s,c="[次]gemini-3-flash-preview",p)=>{const m=E($,p,c),u=s.novelContent.substring(0,5e3);let o="";s.episodeId>1&&s.previousScriptContext&&(o=`4. **上一集结尾**（用于检查衔接）：
...
`;try{const t=e.length;let l=O(t,c);t>15e4?l=c.includes("claude")?12e5:9e5:t>1e5?l=c.includes("claude")?9e5:72e4:t>5e4?l=c.includes("claude")?6e5:48e4:l=c.includes("claude")?3e5:18e4;const d=await _(m,c,[{role:"user",content:i}],{temperature:.5,responseFormat:{type:"json_object"},timeout:l});let r=I(d.text,"AuditScript");return r&&!Array.isArray(r)&&r.items&&(r=r.items),Array.isArray(r)||(r=[]),r.map((h,n)=>({...h,id:Date.now().toString()+n,status:"pending"}))}catch(t){return console.error("Audit Script Error:",t),[]}},v=async($,e,s,c,p,m="[次]gemini-3-pro-preview-thinking")=>{const u=E($,p,m);console.log(`[Deep Audit] 开始审稿 - 模型: ${m}, 剧本长度: ${e.length}字`),console.log(`[Deep Audit] 小说上下文长度: ${s.length}字, 大纲上下文长度: ${c.length}字`);const o=e.length+s.length+c.length;console.log(`[Deep Audit] 总输入长度: ${o}字`);const a=m.includes("opus");m.includes("claude");let i=s,t=c;if(a)console.log("[Deep Audit] ✅ 使用 Claude Opus 模型，支持超长上下文，无需压缩");else{if(s.length>5e3){console.log(`[Deep Audit] ⚠️ 小说上下文过长 (${s.length}字)，当前模型不支持超长上下文`),console.log("[Deep Audit] 建议：切换到 Claude Opus 4.6 (MixAI) ⭐ 模型以获得完整评估"),console.log("[Deep Audit] 当前将压缩到 5000 字（仅供参考，可能影响评估准确性）");const h=Math.floor(5e3/2);i=s.substring(0,h)+`
...
`;try{const d=e.length;let r=O(d,m);const h=m.includes("opus"),n=m.includes("claude");d>15e4?h?r=18e5:n?r=12e5:r=9e5:d>1e5?h?r=12e5:n?r=9e5:r=72e4:d>5e4?r=n?6e5:48e4:r=n?3e5:18e4,console.log(`[Deep Audit] 超时设置: ${r/1e3}秒 (${Math.round(r/6e4)}分钟)`);const S=await _(u,m,[{role:"user",content:l}],{temperature:.4,responseFormat:{type:"json_object"},timeout:r});console.log("[Deep Audit] API 调用成功，开始解析结果"),console.log(`[Deep Audit] 响应长度: ${S.text.length} 字符`);const y=m.toLowerCase().includes("claude"),f=await b(S.text,"Deep Audit",$,p,y),T=(f.annotations||[]).map((g,A)=>({...g,id:`anno_${Date.now()}_${A}`,status:"pending"}));return console.log(`[Deep Audit] 审稿完成 - 总分: ${f.totalScore}, 批注数: ${T.length}`),{scriptId:Date.now().toString(),totalScore:f.totalScore||0,dimensionScores:f.dimensionScores||[],annotations:T,summary:f.summary||"",timestamp:Date.now()}}catch(d){throw console.error("[Deep Audit] 审稿失败:",d),console.error("[Deep Audit] 错误详情:",{message:d.message,status:d.status,name:d.name}),d}},V=async($,e,s,c,p,m="claude-opus-4-6-a",u)=>{const a=e.length;if(console.log(`[Batched Deep Audit] 开始分批审稿 - 总长度: ${a}字, 模型: ${m}`),a<=2e5)return console.log("[Batched Deep Audit] 剧本长度未超过20万字，使用普通审稿"),v($,e,s,c,p,m);const i=[];for(let n=0;n<a;n+=8e4)i.push(e.substring(n,Math.min(n+8e4,a)));console.log(`[Batched Deep Audit] 分为 ${i.length} 批次处理`);const t=[],l=new Map;let d=0;for(let n=0;n<i.length;n++){const S=i[n],y=n*8e4,f=Math.min(y+8e4,a);u&&u(n+1,i.length,`正在审稿第 ${n+1}/${i.length} 批次 (${y}-${f}字)`),console.log(`[Batched Deep Audit] 处理第 ${n+1}/${i.length} 批次`);try{const T=await v($,S,s,c,p,m);T.annotations.forEach(g=>{t.push({...g,id:`batch${n}_${g.id}`,location:{...g.location,lineContent:`[第${n+1}批] ${g.location.lineContent}`}})}),T.dimensionScores.forEach(g=>{const A=l.get(g.dimension);A?(A.total+=g.score,A.count+=1):l.set(g.dimension,{total:g.score,count:1})}),d+=T.totalScore}catch(T){console.error(`[Batched Deep Audit] 第 ${n+1} 批次失败:`,T)}}const r=Math.round(d/i.length),h=Array.from(l.entries()).map(([n,S])=>({dimension:n,score:Math.round(S.total/S.count),comment:`基于 ${S.count} 个批次的平均分数`}));return console.log(`[Batched Deep Audit] 分批审稿完成 - 总批注数: ${t.length}, 平均分: ${r}`),{scriptId:Date.now().toString(),totalScore:r,dimensionScores:h,annotations:t,summary:`【分批审稿报告】本剧本共 ${a} 字，分 ${i.length} 批次审核完成。共发现 ${t.length} 处问题。`,timestamp:Date.now()}},K=async($,e,s,c,p,m="claude-opus-4-6-a",u)=>{var r,h;const a=e.length;console.log(`[Incremental Audit] 开始增量式审稿 - 总长度: ${a}字, 模型: ${m}`);const i=[];for(let n=0;n<a;n+=5e4)i.push(e.substring(n,Math.min(n+5e4,a)));console.log(`[Incremental Audit] 分为 ${i.length} 批次处理`);let t={totalScore:0,dimensionScores:[],summary:"",keyIssues:[],overallTrend:""};const l=[],d=E($,p,m);for(let n=0;n<i.length;n++){const S=i[n],y=n*5e4,f=Math.min(y+5e4,a);u&&u(n+1,i.length,`正在审稿第 ${n+1}/${i.length} 段 (${y}-${f}字)`),console.log(`[Incremental Audit] 处理第 ${n+1}/${i.length} 段`);const T=`
# Role
...
["方案一内容...", "方案二内容...", "方案三内容..."]
`,a=await _(u,"[次]gemini-3-flash-preview",[{role:"user",content:o}],{responseFormat:{type:"json_object"},timeout:6e4,maxRetries:1});return R(a.text)},W=async($,e,s,c)=>{const m=E($,c,"[次]gemini-3-pro-preview-thinking"),o=`
# Task
请对以下剧本进行**全篇批量优化**。
【优化指令】：${L(s)}
...
`;return(await _(m,"[次]gemini-3-pro-preview-thinking",[{role:"user",content:o}],{timeout:12e4,maxRetries:1})).text||e},Q=async($,e,s,c="[次]gemini-3-pro-preview-thinking")=>{const p=E($,s,c),u=`
# Role
...
`;try{const o=e.reduce((l,d)=>l+d.content.length,0);let a=O(o,c);o>15e4?a=c.includes("claude")?12e5:9e5:o>1e5?a=c.includes("claude")?9e5:72e4:o>5e4?a=c.includes("claude")?6e5:48e4:a=c.includes("claude")?3e5:18e4;const i=await _(p,c,[{role:"user",content:u}],{temperature:.4,responseFormat:{type:"json_object"},timeout:a});return{...I(i.text,"AuditSingleEpisode"),timestamp:Date.now()}}catch(o){throw console.error("Script Comparison Failed",o),o}},q=({oldText:$,newText:e,containerClassName:s,loadingText:c="正在计算差异..."})=>{const[p,m]=C.useState(null);return C.useEffect(()=>{let u=!1;return m(null),M(()=>import("./diff-vendor-BTAiA4S9.js"),[]).then(o=>{u||m(o.diffWordsWithSpace($,e))}),()=>{u=!0}},[$,e]),N.jsx("div",{className:s,children:p?p.map((u,o)=>u.added?N.jsx("span",{className:"bg-green-100 text-green-800 border-b-2 border-green-300 rounded px-0.5",children:u.value},o):u.removed?N.jsx("span",{className:"bg-red-100 text-red-800 line-through decoration-red-400 opacity-60 select-none px-0.5",children:u.value},o):N.jsx("span",{className:"text-gray-800",children:u.value},o)):N.jsx("div",{className:"text-gray-400",children:c})})},X=$=>new Promise(e=>setTimeout(e,$)),j=async($,e,s,c)=>{var i,t;const p=(l,d="info")=>{var r;(r=s.onLog)==null||r.call(s,{level:d,message:l,timestamp:Date.now()})},m={signal:c.signal,log:p,setProgress:l=>{var d;return(d=s.onProgress)==null?void 0:d.call(s,l)}},u=e.retries??0,o=e.retryDelayMs??600;let a=0;for(;a<=u;)try{a+=1,a>1&&p(`第 ${a} 次执行任务`,"warn");const l=await $(m);return(i=s.onProgress)==null||i.call(s,null),l}catch(l){if(c.signal.aborted)throw p("任务已取消","warn"),l;if(a<=u){p(`任务失败，准备重试（${a}/${u}）`,"warn"),await X(o);continue}if(e.fallbackTask){p("主任务失败，执行降级任务","warn");const d=await e.fallbackTask(l,m);return(t=s.onProgress)==null||t.call(s,null),d}throw p(l instanceof Error?l.message:"任务执行失败","error"),l}throw new Error("任务执行失败")},Y=$=>{const[e,s]=C.useState(!1),[c,p]=C.useState(null),[m,u]=C.useState(null),[o,a]=C.useState([]),i=C.useRef(null),t=C.useRef(null),l=C.useCallback(n=>{a(S=>[...S.slice(-39),n])},[]),d=C.useCallback(()=>{var n;(n=i.current)==null||n.abort()},[]),r=C.useCallback(async(n,S={})=>{var T;(T=i.current)==null||T.abort();const y=new AbortController;i.current=y,s(!0),p(null),u(null),l({level:"info",message:`开始任务：${$}`,timestamp:Date.now()});const f=async()=>j(n,S,{onLog:l,onProgress:u},y);t.current=f;try{const g=await f();return l({level:"info",message:`完成任务：${$}`,timestamp:Date.now()}),g}catch(g){const A=g instanceof Error?g:new Error(String(g));throw p(A),A}finally{i.current===y&&(i.current=null),s(!1),u(null)}},[l,$]),h=C.useCallback(async()=>t.current?t.current():null,[]);return{cancel:d,error:c,isRunning:e,logs:o,progress:m,retryLastTask:h,runTask:r}};export{q as D,Z as a,V as b,v as c,Q as d,W as e,z as f,G as g,K as p,J as r,Y as u};
